...
df = pd.read_excel(r"C:\Users\Administrator\Nutstore\2\我的坚果云\博士研究工作\刘天罡组合作项目\比较基因组学信息分析\GO KEGG注释\GO注释\SNP GO注释\SNP GO anno_total.xlsx",0)
...
# 计算每个 GO term 的数量
df_count = df_long.groupby(['GO_category', 'GO_term']).size().reset_index(name='count')

# 按 GO 大类和数量排序
df_count = df_count.sort_values(['GO_category', 'count'], ascending=[True, False])
...
# 绘制横向柱状图
bars = plt.barh(range(len(df_count)), df_count['count'], color='gray')  # 初始颜色设置为灰色
